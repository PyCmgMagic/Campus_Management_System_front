...
public class editCourseController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        semesterComboBox.setItems(Data.getInstance().getSemesterList());
        semesterComboBox.getSelectionModel().select(Data.getInstance().getCurrentTerm());
    ...
    public void loadCourseDetails(int courseId) {
        ...
        NetworkUtils.get("/class/detail/" + courseId, new NetworkUtils.Callback<String>() {
            ...
            public void onFailure(Exception e) {
                ...
                ShowMessage.showErrorMessage("网络错误: " ,res.get("msg").getAsString());
            }
        });
    }
    private  void updateUI(){
        ...
        endWeekField.setText(courseData.get("weekEnd").getAsString());
        classHoursField.setText(courseData.get("period").getAsString());
        departmentField.setText(courseData.get("college").getAsString());
        semesterComboBox.getSelectionModel().select(courseData.get("term").getAsString());
        courseTypeComboBox.getSelectionModel().select(courseData.get("type").getAsString());
        if(courseData.get("examination").getAsInt()==1){
            assessmentTypeComboBox.getSelectionModel().select("考试");
        }else if(courseData.get("examination").getAsInt()== 0){
            assessmentTypeComboBox.getSelectionModel().select("考查");
        }
        regularPercentageField.setText(courseData.get("regularRatio").getAsDouble()*100 + "");
        finalPercentageField.setText(courseData.get("finalRatio").getAsDouble()*100 +"");
        courseDescriptionField.setText(courseData.get("intro").getAsString());
    ...
    private void handleSubmit(ActionEvent event) {
        if (validateForm()) {
            ...
            NetworkUtils.post("/class/update/" + courseId, requestJson, new NetworkUtils.Callback<String>() {
                @Override
                public void onSuccess(String result) {
